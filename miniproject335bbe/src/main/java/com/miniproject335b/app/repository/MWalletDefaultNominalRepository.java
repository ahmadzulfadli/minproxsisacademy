...
public interface MWalletDefaultNominalRepository extends JpaRepository<MWalletDefaultNominal, Long>{
    ...
    @Query(value = "SELECT * FROM m_wallet_default_nominal WHERE is_delete = ?1 ORDER BY id ASC", nativeQuery = true)
    List<MWalletDefaultNominal> findByIsDelete(Boolean isDelete);
}
